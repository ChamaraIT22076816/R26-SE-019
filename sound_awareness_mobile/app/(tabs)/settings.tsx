...
export default function SettingsScreen() {
  ...
  const [backgroundListening, setBackgroundListening] = useState(true);
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 100 }]}>
        ...
        <View style={styles.sectionCard}>
          <SettingToggle
            ...
            description="Vibrate phone on sound detection"
          ...
          <SettingToggle
            ...
            iconColor="#FFB020"
            label="Critical Alert Flashlight"
            description="Flash camera LED on critical threats"
            value={flashlight}
            onValueChange={setFlashlight}
          />
          <View style={styles.settingDivider} />
          <SettingToggle
            icon="ear-outline"
            iconColor="#6B8AFF"
            label="Background Listening Mode"
            description="Continue monitoring when app is minimized"
            value={backgroundListening}
            onValueChange={setBackgroundListening}
          />
        </View>
          ...
          <View style={styles.sensitivityHeader}>
            ...
            <View style={styles.sensitivityHeaderText}>
              ...
              <Text style={styles.settingDesc}>
                {sensitivity <= 2 ? 'Fewer false positives, may miss quiet sounds' : sensitivity >= 4 ? 'Catches more sounds, may trigger more alerts' : 'Balanced detection for most environments'}
              </Text>
            </View>
          </View>
          <SensitivitySlider value={sensitivity} onValueChange={setSensitivity} />
          <View style={styles.sensitivityBadge}>
            <Ionicons name="speedometer-outline" size={14} color={sensitivity >= 4 ? '#FF6B8A' : sensitivity <= 2 ? '#4DA6FF' : '#FFB020'} />
            <Text style={[styles.sensitivityBadgeText, { color: sensitivity >= 4 ? '#FF6B8A' : sensitivity <= 2 ? '#4DA6FF' : '#FFB020' }]}>
              Level {sensitivity} — {sensitivity <= 2 ? 'Conservative' : sensitivity >= 4 ? 'Aggressive' : 'Balanced'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.sectionCard}>
          <SettingToggle
            ...
            iconColor="#FF3B5C"
            label="SOS Auto-Call"
            description="Auto-dial emergency services on SOS dispatch"
            value={autoCall}
          ...
          <SettingToggle
            ...
            description="Include GPS coordinates in SOS alerts"
            value={shareLocation}
          ...
          <SettingToggle
            icon="moon-outline"
            iconColor="#C084FC"
            label="Night Mode Sensitivity"
            description="Increase sensitivity during nighttime hours"
            value={nightMode}
